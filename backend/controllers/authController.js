...
const createEmailTransporter = () => {
  ...
  if (!emailUser || !emailPass || /your-gmail-address|your-gmail-app-password/i.test(`${emailUser} ${emailPass}`)) {
...
export const updateUser = async (req, res) => {
  ...
  // Ownership check supports both Firebase (uid) and backend JWT (_id)
  if (req.user) {
  ...
  try {
    ...
    const canVerify = await canVerifyFirebaseTokens();

    // Update Firebase profile only when uid is a Firebase UID and Admin SDK available
    if (!isObjectId && canVerify) {
      try {
        await admin.auth().updateUser(uid, { displayName, email, photoURL });
      } catch (e) {
        console.log('⚠️ Skipping Firebase update:', e.message);
      }
    } else if (!isObjectId && !canVerify) {
      console.log('⚠️ Firebase Admin not configured, skipping admin.auth().updateUser');
    }
    ...
    const filter = isObjectId ? { _id: uid } : { firebaseUid: uid };
    const updatedUser = await User.findOneAndUpdate(filter, updateDoc, { new: true });
...
export const getUserProfile = async (req, res) => {
  ...
  try {
    ...
    if (!isObjectId && canVerify) {
      ...
      console.log('⚠️ Firebase Admin not configured, proceeding with MongoDB user only');
    }

    const user = isObjectId ? await User.findById(uid) : await User.findOne({ firebaseUid: uid });
